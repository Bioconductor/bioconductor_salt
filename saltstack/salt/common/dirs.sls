...
{% set build_version =  "%.2f" | format(build.version) %}

{%- for type in build.types %}
make_{{ build_version }}_{{ type }}_directory:
  file.directory:
    - name: {{ machine.user.home }}/{{ machine.user.name }}/bbs-{{ build_version }}-{{ type }}/log
...
make_{{ build_version }}_bioc_rdownloads:
  file.directory:
    - name: {{ machine.user.home }}/{{ machine.user.name }}/bbs-{{ build_version }}-bioc/rdownloads
    - user: {{ machine.user.name }}
    - group: {% if grains['os'] == 'MacOS' %}staff{% else %}{{ machine.user.name }}{% endif %}
    - makedirs: True
    - replace: False
...
make_{{ build_type }}_src_contrib:
  file.managed:
    - name: {{ machine.user.home }}/biocpush/PACKAGES/{{ build_version }}/{{ build_type|replace('-', '/') }}/src/contrib/PACKAGES
    - user: biocpush
...
make_{{ build_type }}_bin_windows_contrib:
  file.managed:
    - name: {{ machine.user.home }}/biocpush/PACKAGES/{{ build_version }}/{{ build_type|replace('-', '/') }}/bin/windows/contrib/{{ r.version[2:] }}/PACKAGES
    - user: biocpush
    - group: {% if grains['os'] == 'MacOS' %}staff{% else %}biocpush{% endif %}
    - makedirs: True
    - dir_mode: 774
...
make_{{ build_type }}_bin_macosx_big-sur-arm64_contrib:
  file.managed:
    - name: {{ machine.user.home }}/biocpush/PACKAGES/{{ build_version }}/{{ build_type }}/bin/macosx/big-sur-arm64/contrib/{{ r.version[2:] }}/PACKAGES
    - user: biocpush
    - group: {% if grains['os'] == 'MacOS' %}staff{% else %}biocpush{% endif %}
    - makedirs: True
    - dir_mode: 774
...
git_bioc_manifest:
  git.cloned:
    ...
    - target: {{ machine.user.home }}/{{ machine.user.name }}/bbs-{{ build_version }}-bioc/{{ repo.manifest.name }}
    - user: {{ machine.user.name }}
